...
export class GameScene {
    constructor(param, event) {
        this.dispose = () => (this.cleared && this.stage.isFinalStage() ? 2 : 0);
        ...
        this.pauseMenu = new Menu([
            ...
            new MenuButton("AUDIO: " + (event.sound.isEnabled() ? "ON " : "OFF"), event => {
                ...
                this.pauseMenu.changeButtonText(3, "AUDIO: " + (event.sound.isEnabled() ? "ON " : "OFF"));
            }),
            new MenuButton("QUIT", event => {
                event.transition.activate(true, 3 /* TransitionEffectType.BoxVertical */, 1.0 / 30.0, event => event.changeScene(TitleScreen));
            })
        ]);
        this.cleared = false;
        this.clearTimer = 0;
        this.startTimer = START_TIME;
        event.transition.activate(false, 2 /* TransitionEffectType.CirleIn */, 1.0 / 30.0, null).setCenter(new Vector2(80, 72));
        this.hintPos = 0;
    ...
    resetTransition(event) {
        event.transition.activate(true, 3 /* TransitionEffectType.BoxVertical */, 1.0 / 20.0, () => this.reset());
    }
    update(event) {
        ...
        if (this.cleared) {
            ...
            if ((this.clearTimer -= event.step) <= 0) {
                if (this.stage.isFinalStage()) {
                    event.transition.activate(true, 1 /* TransitionEffectType.Fade */, 1.0 / 60.0, event => event.changeScene(Intro), [255, 255, 255], 4);
                }
                else {
                    event.transition.activate(true, 2 /* TransitionEffectType.CirleIn */, 1.0 / 30.0, () => this.nextStage(), [0, 0, 0])
        ...
        if (event.keyboard.getActionState("back") == 3 /* State.Pressed */) {
            event.sound.playSequence(SoundSource.Choose, 0.60, "square");
            this.stage.undo();
        }
        else if (event.keyboard.getActionState("restart") == 3 /* State.Pressed */) {
            event.sound.playSequence(SoundSource.Select, 0.60, "square");
            this.resetTransition(event);
        }
    }
    drawClear(canvas) {
        ...
        canvas.drawBitmapRegion(bmp, 0, 0, bmp.width, bmp.height / 2, canvas.width / 2 - bmp.width / 2, canvas.height / 2 - bmp.height / 2 - y);
        canvas.drawBitmapRegion(bmp, 0, bmp.height / 2, bmp.width, bmp.height / 2, canvas.width / 2 - bmp.width / 2, canvas.height / 2 + y);
    }
    drawControlsScreen(canvas, y) {
        if (this.startTimer <= 0) {
            canvas.setFillColor(0, 0, 0, 0.67);
            canvas.fillRect();
        }
    ...
    redraw(canvas) {
        ...
        if (this.startTimer > 0) {
            canvas.setFillColor(0, 0, 0, 0.67);
            canvas.fillRect();
            canvas.drawText(fontYellow, "STAGE " + String(this.stage.index), canvas.width / 2, canvas.height / 2 - 8, 0, 0, true);
        }
        let y;
        let t;
        ...
        if (this.pauseMenu.isActive()) {
            canvas.setFillColor(0, 0, 0, 0.67);
            canvas.fillRect();
            this.pauseMenu.draw(canvas, 0, 0, 0, 13, true);
            for (let i = 0; i < 2; ++i) {
                canvas.drawText(fontYellow, HINT, canvas.width / 2 - this.hintPos + HINT.length * 8 * i, 144 - 10, 0, 0, true);
            }
        }
        if (this.cleared) {
