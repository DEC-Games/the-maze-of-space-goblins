...
export class Intro {
    ...
    updateDust(event) {
        ...
        if ((this.dustTimer += event.step) >= DUST_GEN_TIME) {
            nextObject(this.dust, Dust)
                .spawn(this.ufoPos.x + 2, this.ufoPos.y + 10, DUST_LIFE_TIME, 4, new Vector2(-2, 0));
    ...
    update(event) {
        ...
        if (this.movePhase == 1) {
            ...
            if ((this.ufoPos.x += MOVE_SPEED * event.step) > 160) {
                event.transition.activate(true, 1 /* TransitionEffectType.Fade */, 1.0 / 30.0, event => {
                    if (this.phase == 0)
    ...
    redraw(canvas) {
        ...
        canvas.drawBitmapRegion(canvas.data.getBitmap("art1"), 48, 0, 16, 16, this.ufoPos.x, Math.round(this.ufoPos.y));
